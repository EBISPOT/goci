...
public class QueryManager {
    ...
    public URI getCytogeneticBandForAssociation(SparqlTemplate sparqlTemplate, URI association) throws
        ...
        List<URI> results =
                sparqlTemplate.query(SparqlQueries.BAND_FOR_ASSOCIATION, new URIMapper("band"), association);
        if (results.size() == 1) {
    ...
    public Set<URI> getAssociationsLocatedInCytogeneticBand(SparqlTemplate sparqlTemplate,
                                                            URI bandIndividual,
                                                            List<Filter> filters) {
        ...
        if (filters.size() == 0) {
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND,
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    Object retrieved = checkCache("getAssociationsLocatedInCytogeneticBand",
                                                  sparqlTemplate,
                                                  bandIndividual,
                                                  filter.getFilteredValues().get(1),
                                                  filter.getFilteredValues().get(0));
                    if (retrieved != null) {
                        return (Set<URI>) retrieved;
                    }

                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_PVALUE_FILTER,
                                                        new URIMapper("association"),
                                                        bandIndividual,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_DATE_FILTER,
                                                        new URIMapper("association"),
                                                        bandIndividual,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_PVALUE_DATE_FILTER,
                                                new URIMapper("association"),
                                                bandIndividual,
                                                pval_max,
                                                pval_min,
    ...
    public Set<URI> getAssociationsLocatedInCytogeneticBand(SparqlTemplate sparqlTemplate,
        ...
        if (filters.size() == 0) {
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_NAME,
                                                new URIMapper("association"),
                                                bandName));
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_NAME_PVALUE_FILTER,
                                                        new URIMapper("association"),
                                                        bandName,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_NAME_DATE_FILTER,
                                                        new URIMapper("association"),
                                                        bandName,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_IN_BAND_NAME_PVALUE_DATE_FILTER,
                                                new URIMapper("association"),
                                                bandName,
                                                pval_max,
                                                pval_min,
    ...
    public Set<URI> getTraitsLocatedInCytogeneticBand(SparqlTemplate sparqlTemplate,
                                                      URI bandIndividual,
                                                      List<Filter> filters) {
        ...
        if (filters.size() == 0) {
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.TRAITS_IN_BAND_PVALUE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandIndividual,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.TRAITS_IN_BAND_DATE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandIndividual,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
        ...
        if (filters.size() == 0) {
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.TRAITS_IN_BAND_NAME_PVALUE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandName,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.TRAITS_IN_BAND_NAME_DATE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandName,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.TRAITS_IN_BAND_NAME_PVALUE_DATE_FILTER,
                                                new URIMapper("trait"),
                                                bandName,
                                                pval_max,
                                                pval_min,
    ...
    public List<URI> getTraitsOrderedByIdentificationDateForBand(SparqlTemplate sparqlTemplate,
                                                                 URI bandIndividual,
                                                                 List<Filter> filters) {
        ...
        if (filters.size() == 0) {
            Object retrieved =
                    checkCache("getTraitsOrderedByIdentificationDateForBand", sparqlTemplate, bandIndividual);
            if (retrieved != null) {
                return (List<URI>) retrieved;
            }

            List<URI> queryResults =
                    sparqlTemplate.query(SparqlQueries.DATE_OF_TRAIT_ID_FOR_BAND,
                                         new URIMapper("trait"),
                                         bandIndividual);
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.DATE_OF_TRAIT_ID_FOR_BAND_PVALUE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandIndividual,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.DATE_OF_TRAIT_ID_FOR_BAND_DATE_FILTER,
                                                        new URIMapper("trait"),
                                                        bandIndividual,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.DATE_OF_TRAIT_ID_FOR_BAND_PVALUE_DATE_FILTER,
                                                new URIMapper("trait"),
                                                bandIndividual,
                                                pval_max,
                                                pval_min,
    ...
    public Set<URI> getAssociationsForTrait(SparqlTemplate sparqlTemplate, URI trait, List<Filter> filters) {
        ...
        if (filters.size() == 0) {
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT,
                                                new URIMapper("association"),
                                                trait));
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_PVALUE_FILTER,
                                                        new URIMapper("association"),
                                                        trait,
                                                        filter.getFilteredValues().get(1),
                                                        filter.getFilteredValues().get(0)));
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_DATE_FILTER,
                                                        new URIMapper("association"),
                                                        trait,
                                                        filter.getFilteredRange().to(),
                                                        filter.getFilteredRange().from()));
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_PVALUE_DATE_FILTER,
                                                new URIMapper("association"),
                                                trait,
                                                pval_max,
                                                pval_min,
    ...
    public List<URI> getAssociationForTraitAndBand(SparqlTemplate sparqlTemplate,
                                                   URI trait,
                                                   URI bandIndividual,
                                                   List<Filter> filters) {
        List<URI> results = new ArrayList<URI>();
        ...
        if (filters.size() == 0) {
            ...
            List<URI> queryResults =
                    sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_AND_BAND,
                                         new URIMapper("association"),
                                         trait,
                                         bandIndividual);
            // de-duplicate results; should be handled by        List<URI> results = new ArrayList<URI>();
            ...
            for (Filter filter : filters) {
                if (filter.getFilteredType().equals(Association.class)) {
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_AND_BAND_PVALUE_FILTER,
                                                        new URIMapper("association"),
                                                        trait,
                                                        bandIndividual,
                                                        filter.getFilteredValues().get(1),
                    ...
                    results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_AND_BAND_DATE_FILTER,
                                                        new URIMapper("association"),
                                                        trait,
                                                        bandIndividual,
                                                        filter.getFilteredRange().to(),
            ...
            results.addAll(sparqlTemplate.query(SparqlQueries.ASSOCIATIONS_FOR_TRAIT_AND_BAND_PVALUE_DATE_FILTER,
                                                new URIMapper("association"),
                                                trait,
                                                bandIndividual,
                                                pval_max,
    ...
    /**
     * Gets the list of all types of the supplied traits, ordered by specificity.  In other words, traits are ordered so
     * that the most specific asserted types are first, followed by each of the traits ancestors from immediate parents
    ...
    public List<URI> getOrderedTraitTypes(SparqlTemplate sparqlTemplate, URI trait) {
        ...
        List<URI> results =
                sparqlTemplate.query(SparqlQueries.PARENTS_AND_DISTANCE_BY_TRAIT, new URIMapper("type"), trait);
