...
CREATE OR REPLACE VIEW WEEKLY_TOTALS_SUMMARY_VIEW (ID, WEEK, ENTRIES, STUDIES)
AS SELECT ROWNUM, V.* FROM (SELECT TRUNC(H.STUDY_ADDED_DATE, 'D') AS WEEK, COUNT(*) AS ENTRIES, COUNT(DISTINCT S.PUBMED_ID) AS STUDIES
