...
################################################################################
Migration script to create MONTHLY_CURATOR_TOTALS_SUMMARY_VIEW and
...
CREATE OR REPLACE VIEW MONTHLY_TOTALS_SUMMARY_VIEW (YEAR, MONTH, CURATOR, CURATOR_TOTAL, CURATION_STATUS, MONTHLY_TOTAL)
AS (SELECT YEAR_E AS YEAR, MONTH_E AS MONTH, CURATOR, CURATOR_TOTAL, CURATION_STATUS, MONTHLY_TOTAL FROM (
...
ORDER BY TRUNC(TO_DATE(PUBLICATION_DATE), 'MONTH') DESC) N on YEAR_N = YEAR_E AND MONTH_N = MONTH_E));

--------------------------------------------------------
--  CREATE YEARLY_CURATOR_TOTALS_SUMMARY_VIEW
--------------------------------------------------------

CREATE OR REPLACE VIEW YEARLY_TOTALS_SUMMARY_VIEW (YEAR, CURATOR, TOTAL, CURATION_STATUS)
AS SELECT EXTRACT (YEAR FROM (TRUNC(TO_DATE(S.PUBLICATION_DATE), 'YEAR'))) AS YEAR,
C.LAST_NAME AS CURATOR,
...
ORDER BY TRUNC(TO_DATE(S.PUBLICATION_DATE), 'YEAR') DESC, C.LAST_NAME;