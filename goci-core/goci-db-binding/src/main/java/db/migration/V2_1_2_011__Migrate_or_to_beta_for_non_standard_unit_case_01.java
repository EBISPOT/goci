 ...
 */
public class V2_1_2_011__Migrate_or_to_beta_for_non_standard_unit_case_01 implements SpringJdbcMigration {
    ...
    private static final String SELECT_ASSOCIATIONS =
            "SELECT ID, OR_PER_COPY_NUM, DESCRIPTION FROM ASSOCIATION \n" +
                    ...
                    "AND (DESCRIPTION  LIKE 'increase in%' OR DESCRIPTION LIKE 'decrease in%')";
    ...
    public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
        ...
        jdbcTemplate.query(SELECT_ASSOCIATIONS, (resultSet, i) -> {
            ...
            // Split description
            String[] output = description.split(" in ",2);
            String direction = output[0].trim();
            String unit = output[1].trim();

            // Run update
            jdbcTemplate.update(UPDATE_ASSOCIATION, betaNum, unit, direction, null, null, id);
            return null;
        });
    }
}