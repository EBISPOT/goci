...
CREATE TABLE ASSOCIATION_REPORT (
  ...
  PUBMED_ID_ERROR NUMBER(19,0),
  SNP_ERROR VARCHAR2(255 CHAR),
  SNP_GENE_ON_DIFF_CHR VARCHAR2(255 CHAR),
  NO_GENE_FOR_SYMBOL VARCHAR2(255 CHAR),
  GENE_NOT_ON_GENOME VARCHAR2(255 CHAR));
...
ALTER TABLE SINGLE_NUCLEOTIDE_POLYMORHISM ADD (
  MERGED NUMBER(19,0,
...
-- UPSTREAM
SELECT SNP_ID, GENE_ID, IS_INTERGENIC, IS_UPSTREAM, IS_DOWNSTREAM, DISTANCE FROM (
...
SELECT SNP_ID, GENE_ID, IS_INTERGENIC, IS_UPSTREAM, IS_DOWNSTREAM, DISTANCE FROM (
  ...
  AND INTERGENIC = 1);
-- INTERGENIC
SELECT SNP_ID, GENE_ID, GENE_NAME, SNP_GENE_SYMBOLS, IS_INTERGENIC, IS_UPSTREAM, IS_DOWNSTREAM, DISTANCE FROM (
  SELECT s.ID AS SNP_ID, g.ID AS GENE_ID, g.GENE AS GENE_NAME, gs.SNP_GENE_SYMBOLS, 1 AS IS_INTERGENIC, 0 AS IS_UPSTREAM, 0 AS IS_DOWNSTREAM, NULL AS DISTANCE
  FROM GWASSTUDIESSNP gs
  JOIN GWASSNPXREF sx ON sx.GWASSTUDIESSNPID = gs.ID
  JOIN GWASSNP s ON sx.SNPID = s.ID
...
UPDATE SINGLE_NUCLEOTIDE_POLYMORPHISM snp SET MERGED = (
  SELECT gs.MERGED
  FROM GWASSTUDIESSNP gs
  JOIN GWASSNPXREF sx ON sx.GWASSTUDIESSNPID = gs.ID
  JOIN GWASSNP s ON sx.SNPID = s.ID
