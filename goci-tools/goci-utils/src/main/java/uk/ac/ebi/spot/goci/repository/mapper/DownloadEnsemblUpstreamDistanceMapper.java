 ...
 *         not include details of upstream gene distance.
 */
@Component
public class DownloadEnsemblUpstreamDistanceMapper implements CatalogDataMapper {
    ...
    @Override public List<CatalogHeaderBinding> getRequiredDatabaseFields() {
        return Arrays.asList(CatalogHeaderBinding.ENSEMBL_MAPPED_GENE,
                             CatalogHeaderBinding.ENSEMBL_UPSTREAM_GENE_DISTANCE);
    }

    @Override public CatalogHeaderBinding getOutputField() {
        return CatalogHeaderBinding.DOWNLOAD_ENSEMBL_UPSTREAM_GENE_DISTANCE;
    }
    ...
    @Override public String produceOutput(Map<CatalogHeaderBinding, String> databaseValues) {
        ...
        if (mapped.isEmpty()) {
            output = databaseValues.get(CatalogHeaderBinding.ENSEMBL_UPSTREAM_GENE_DISTANCE);
        }
        else {
            output = "";
        }
