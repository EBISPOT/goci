...
var studies_MAP = buildMap(studies);
var FEATURES_MAP = buildMap(FEATURES);
//CLEAN STUDY DATA, replace null,na,undefined,false to 0, true to 1
var studies_MAP_clean = new Map(studies_MAP)
studies_MAP_clean.forEach(function(study, id) {
    study.forEach(function(value, feature) {
        if(!FEATURES_MAP.has(feature)){
            study.delete(feature);
        }else{
            //clean
            var value = study.get(feature);
            //clean
            if(value == undefined | value==null | value === 'false' | value == "NA") {
                value = 0;
            }
...
//        Array.from(FEATURES_MAP.keys()).map((feature)=>{
//
//        })
//
//        if(Object.keys(FEATURES_MAP).indexOf(feature) == -1){
//            delete studies_clean[study_id][feature]
//        }
