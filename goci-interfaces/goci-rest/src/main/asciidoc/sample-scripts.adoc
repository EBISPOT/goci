...
If for example you want to use the REST API to retrieve all association data for a number of studies given a list of Pubmed IDs, you would first search the Study endpoint for each ID using link:../api/studies/search/findByPubmedId?pubmedId=17463246[api/studies/search/findByPubmedId?pubmedId=17463246].
...
As a simplification, we provide for each study a link to an link:../api/studies/GCST000028/associations?projection=associationByStudy["associationByStudy" projection], which presents a view of all the associations for this and their dependent information including traits and SNPs with genomic mappings.
...
For users interested primarily in the Study endpoint, we also provide a link:../api/studies/GCST000028?projection=study["study" projection], which has all EFO trait information embedded directly in the Study json rather than requiring an additional API call.
