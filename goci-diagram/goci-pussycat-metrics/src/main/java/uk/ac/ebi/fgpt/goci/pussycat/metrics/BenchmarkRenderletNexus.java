...
public class BenchmarkRenderletNexus implements RenderletNexus {
    ...
    public BenchmarkRenderletNexus(){
...
//check if the chromosomes have already been rendered, otherwise render them
        ...
        bm_log.info("Performing rendering for OWLClassExpression " + classExpression);

        long start, end;
        start = System.currentTimeMillis();
        //getLog().debug("Obtaining OWL individuals from reasoner");
        bm_log.info("Obtaining OWL individuals from the reasoner at " + DateTimeStamp.getCurrentTimeStamp());
        Set<OWLNamedIndividual> individuals = reasoner.getInstances(classExpression, false).getFlattened();
        bm_log.info("Reasoner query finished at " + DateTimeStamp.getCurrentTimeStamp());
        end = System.currentTimeMillis();
        double time = ((double) (end - start)) / 1000;
        bm_log.info(individuals.size() + " OWL individuals acquired for OWLClassExpression " + classExpression + "  in " + time + " s.");

        //getLog().debug("There are " + individuals.size() + " owl individuals that satisfy the expression " +  classExpression);
...
//if the map is empty but OWLClassExpression isn't OWLThing, do a dummy rendering to rebuild the map
            if(!classExpression.isOWLThing()){
                getLog().debug("Empty maps and a subset to be rendered - rebuild maps via dummy OWLThing rendering");
    ...
    public void renderFromExistingSVG(SVGBuilder builder, OWLOntology ontology, Set<OWLNamedIndividual> individuals){
        bm_log.info("Rendering from existing SVG. Starting at " + DateTimeStamp.getCurrentTimeStamp());
        OWLClass chromosome = ontology.getOWLOntologyManager()
                .getOWLDataFactory()
                .getOWLClass(IRI.create(OntologyConstants.CHROMOSOME_CLASS_IRI));
        NodeSet<OWLClass> all = reasoner.getSubClasses(chromosome, true);
        ...
        bm_log.info("Rendering chromosomes");
        long start, end;
        start = System.currentTimeMillis();
        ...
        double time = ((double) (end - start)) / 1000;
        bm_log.info("Chromsomes rendered in " + time + " s.");
        ...
        bm_log.info("Building the local band map");
        start = System.currentTimeMillis();
        ...
        bm_log.info("Local map built in " + time + " s.");

        Set<String> bands = currentBands.keySet();

        bm_log.info("Starting to gather SVG elements");
        start = System.currentTimeMillis();
    ...
    public void renderSVGFromScratch(SVGBuilder svgBuilder, OWLOntology ontology, Set<OWLNamedIndividual> individuals){
        ...
        time = ((double) (end - start)) / 1000;
        bm_log.info("Time required to process chromosome SVG " + time + "s");

        bm_log.info("Building data map");
        start = System.currentTimeMillis();
        ...
        time = ((double) (end - start)) / 1000;
        bm_log.info("Time required to build data map " + time + "s");

        bm_log.info("Starting rendering of trait associations at " + DateTimeStamp.getCurrentTimeStamp());
        ...
        bm_log.info("Finished all SVG generation at " + DateTimeStamp.getCurrentTimeStamp());
    }
